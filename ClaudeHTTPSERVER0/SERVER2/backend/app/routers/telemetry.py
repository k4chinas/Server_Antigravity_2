...
async def websocket_telemetry(ws: WebSocket):
    """
    Frontend WebSocket bağlantı noktası.

    Bağlantı kurulunca istemci bekler; yeni telemetri
    POST'u geldiğinde veri otomatik olarak push'lanır.

    Ping/Pong: Uvicorn/Starlette seviyesinde otomatik yönetilir.
    """
    await manager.connect(ws)
    try:
        # Bağlantı açık kaldığı sürece döngüde kal.
        # Frontend'den mesaj beklenmez; sadece bağlantı canlılığı kontrol edilir.
        while True:
            # receive_text() → bağlantı kapanınca WebSocketDisconnect fırlatır
